/**
 * DOM树导出工具 - 一体化版本（All-in-One）Power by slzhou10
 * 
 * 集成了以下三个模块：
...
(function() {
    ...
    function getElementAttributes(element) {
        ...
        const keepAttrs = ['id', 'class', 'name', 'type', 'href', 'src', 'alt', 
                          ...
                          'disabled', 'readonly', 'aria-expanded'];
        ...
        // select / combobox: 采集选项数量（级联场景下选项数变化是关键信号）
        const tagLower = element.tagName.toLowerCase();
        if (tagLower === 'select' && element.options) {
            attrs['options_count'] = element.options.length;
        } else if (tagLower === 'input' && element.getAttribute('role') === 'combobox') {
            const listId = element.getAttribute('aria-controls') || element.getAttribute('list');
            if (listId) {
                const listEl = element.ownerDocument.getElementById(listId);
                if (listEl) {
                    attrs['options_count'] = listEl.children.length;
                }
            }
        }
    ...
    async function getDOMTreeAsync(rootElement = document.body, userConfig = {}, onProgress) {
...
(function() {
    ...
    async function exportDOMTreeWithXPath(filename, userConfig = {}) {
        const config = {
            debug: true,
            needDownload: false,
            checkViewport: true,
        ...
        const result = await window.getDOMTreeAsync(document.body, config);
...
console.log('📖 使用方法：');
console.log('');
console.log(`await exportDOMTreeWithXPath("file.json", {
  enableXPath: true,
  needDownload: true,
  debug: true,
  onlyVisible: true,
...
console.log('');
console.log('💡 v2.3.4 特性：');
console.log('  - ✅ iframe XPath 稳定性：自动处理 src 随机参数');
console.log('  - ✅ 100%节点匹配，无孤儿节点');
console.log('  - 🚀 完美支持多层iframe嵌套（任意深度）');
console.log('  - 📊 XPath数据包含完整的iframe路径链（iframeChain）');
console.log('  - 🔍 详细的调试和诊断信息');
console.log('  - ⚡ 支持主文档 -> iframe A -> iframe B -> ... -> 元素');
console.log('');
console.log('==================================================');


/**
 * 📌 XPath数据格式说明（v2.2.10 - 支持多层iframe嵌套）
 * 
 * mockIdToXPath 的值格式：
 * {
 *   inIframe: boolean,           // 是否在iframe内
 *   iframeChain: string[]|null,  // iframe路径链（从外到内）
 *   xpath: string                // 元素的XPath（相对于所在document）
 * }
 * 
 * 使用示例1（定位iframe内的元素 - 单层）：
 * 
 * const xpathData = mockIdToXPath['abc123'];
 * // xpathData = { inIframe: true, iframeChain: ["//*[@id='chatBot']"], xpath: "//div[@class='content']" }
 * 
 * if (xpathData.inIframe && xpathData.iframeChain) {
 *   let currentDoc = document;
 *   
 *   // 逐层进入iframe
 *   for (const iframeXPath of xpathData.iframeChain) {
 *     const iframe = currentDoc.evaluate(
 *       iframeXPath,
 *       currentDoc,
 *       null,
 ...
 *     
 *     if (!iframe) {
 *       console.error('找不到iframe:', iframeXPath);
 *       break;
 *     }
 *     
 ...
 *   
 *   // 在最内层iframe内定位元素
 *   const element = currentDoc.evaluate(
 *     xpathData.xpath,
 *     currentDoc,
 *     null,
 *     XPathResult.FIRST_ORDERED_NODE_TYPE,
 ...
 * }
 * 
 * 使用示例2（定位多层嵌套iframe内的元素）：
 * 
 * // 假设: 主文档 -> iframe A -> iframe B -> iframe C -> 元素
 * // xpathData = { inIframe: true, iframeChain: ["xpath_A", "xpath_B", "xpath_C"], xpath: "xpath_element" }
 * 
 * // 封装成通用函数
 * function locateElement(xpathData) {
 *   let currentDoc = document;
 *   
 *   if (xpathData.inIframe && xpathData.iframeChain) {
 *     for (const iframeXPath of xpathData.iframeChain) {
 *       const iframe = currentDoc.evaluate(iframeXPath, currentDoc, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
 *       if (!iframe) return null;
 *       currentDoc = iframe.contentDocument || iframe.contentWindow.document;
 *     }
 *   }
 *   
 *   return currentDoc.evaluate(xpathData.xpath, currentDoc, null, XPathResult.FIRST_ORDERED_NODE_TYPE, null).singleNodeValue;
 * }
 * 
 * const element = locateElement(mockIdToXPath['abc123']);
 */
