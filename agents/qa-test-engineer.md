...
### 用例质量 10 维度自检（D1，交付前必过）⭐

用例设计完、交给 Gatekeeper 前，按 10 维度自检（借鉴 oec testcase-review）。
**这是 Designer 的自检清单，不替代 Gatekeeper 的独立判定**——目的是把低级质量问题
在设计阶段就挡掉，不要带进门禁。逐维度过，发现问题就地修：

| # | 维度 | 自检要点 |
|---|------|---------|
| 1 | 需求覆盖度 | 逐需求点 → 用例映射，无未覆盖的需求点（无需求时看正向流是否自洽） |
| 2 | 等价类与边界值 | 每个输入字段的上限/下限/刚好/超限是否齐全 |
| 3 | 反向场景 | 异常分支是否有对应用例（不只测 happy path） |
| 4 | 前置条件完整性 | 依赖数据/环境/权限是否在 preconditions 说清 |
| 5 | 操作步骤可执行性 | steps 是否原子、可复现、顺序清楚 |
| 6 | 预期结果可验证性 | assertions 有明确观测点、能量化判定（非"应该正常"这种模糊话） |
| 7 | 优先级合理性 | P0/P1/P2 与业务风险匹配（核心流程不该标 P2） |
| 8 | 测试类型正确性 | level（unit/integration/system）归类合理 |
| 9 | 标题与颗粒度 | 标题表意、一例一验证点（不要一条塞多个验证） |
| 10 | 重复/冗余 | 合并相似用例、无重复标题、无冗余步骤 |

**轻量原则**：不产出独立评审报告（那是 oec 平台流程），自检发现问题直接改用例；
确实拿不准的标 `state: review` 在 notes 写疑点，交 Gatekeeper 复核。
