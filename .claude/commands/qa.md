...
### 步骤 2.5：需求预审门禁（W2，执行前 6 维度检查）⭐

**在设计用例前，先审需求是否够清楚**。Gatekeeper 是执行后判定，这里补一道
执行前门禁——需求不清就设计，必然返工（StudySkill 事故的另一面）。

借鉴 oec requirement-reviewer，按 6 维度检查需求充分性：

| 维度 | 检查 | 缺失影响 |
|------|------|---------|
| 目标对象 | 测什么页面/接口/模块清楚吗 | 不知道测什么 |
| 关键操作 | 用户/调用方要做哪些操作 | 漏主流程步骤 |
| 完成标志 | 怎样算成功（可观测的结果） | 断言无依据 |
| 数据字段 | 涉及哪些输入/输出字段及约束 | 漏边界/校验用例 |
| 异常场景 | 有哪些已知失败/异常分支 | 只测 happy path |
| 环境信息 | 依赖的环境/账号/前置数据 | 跑不起来 |

**判定**：
- ✅ 6 维度都充分 → 直接进步骤 3
- ⚠️ 轻微缺失（个别维度模糊）→ 基于已有信息合理补全，在结果里注明"已按 X 假设补全"
- ❌ 严重缺失（目标对象/完成标志这种核心维度缺）→ **暂停，向用户追问补充**，
  不要带着模糊需求硬设计用例

L0/L4（单点/缺陷验证）可简化此步；L1/L2/L3 建议执行。
