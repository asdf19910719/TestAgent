...
#### 问题 3.5：数据流完整性 — "步骤间的产出真的传给下游了吗？"⭐

**背景**：StudySkill 事故——LLM 分析有输出、草案显示了（测试全过），但结果没写回云端，下游概念预热拿到空数据生成泛泛内容。单点测试只验证"有输出"，没验证"输出被下游正确消费"。

**检查步骤**：
1. 从需求独立识别本 feature 是否有"跨步骤数据流"（A 产出 → B 消费），例如：
   - 分析结果 → 持久化 → 下游读取
   - 表单提交 → 保存 → 详情页展示
2. 对每个交接点，检查用例断言里**是否真的验证了 B 拿到 A 的产出**

**判定规则**：
- 用例只断言"A 有输出"（如 `result_length > 0`）但无"B 消费成功"断言 → 质疑："数据流交接点 X 未验证，A 的产出可能没传给 B"
- 涉及持久化的功能，无"读回验证"（如查云端 `project.description`）→ 质疑："只验证写入前端状态，未验证持久化"
- 涉及 UI 展示，无可见性断言（`isVisible()`）→ 质疑："数据存在不代表用户能看到"

**追问模板**：
```
功能涉及: LLM分析 → project.description → 概念预热
检查:
- 有断言验证 project.description 被写入吗？ → 无 → 质疑数据流断点
- 有断言验证概念预热 prompt 包含材料吗？  → 无 → 质疑下游消费
结论: 数据流交接点未覆盖，即使用例全过也不能保证主流程通畅
```
